...
void test1() {
    ...
    FileHelper helper;
    helper.genRawFile("test1.dat", 100, (uint64_t)5, double(0)); // 生成5个run，每个包含100个int
    std::string filePath = "./files/test1.dat";
    ...
    try {
        ...
        bool result = helper.readHeader(fs, &header);
        ...
        auto positions = helper.scan(fs, 50, 1, begin_ptr, end_ptr);
        ...
        auto end_read_pos = helper.scan(fs, start_pos, end_pos, 20, begin_ptr2, end_ptr2);
...
void test2() {       
    ...
    int run_length = 100;
    FileHelper helper;
    std::streampos put_pos = helper.genEmptyFile("test2.dat", run_count, int(0));
    std::cout<<"空文件创建完成!"<<std::endl;
    std::string filePath = "./files/test2.dat";
    std::fstream fs(filePath, std::ios::in | std::ios::out | std::ios::binary);
    try {
        ...
        for(int i = 1; i <= run_count; i++) {
            ...
            put_pos = helper.put(fs, put_pos, run_length, i, data, data+run_length);
        }
        ...
        int* end = data1.data();
        auto position = helper.scan(fs, 10, 1, begin, end);
        ...
        auto current = helper.scan(fs, position.first, position.second, 10, begin, end);
